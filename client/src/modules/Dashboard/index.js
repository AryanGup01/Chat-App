...
function findConversationIdByReceiverId(conversations, receiverIdToCheck) {
	// Iterate over the conversations array
	for (const conversation of conversations) {
		// Check if the current conversation's user.receiverId matches the receiverIdToCheck
		if (conversation.user && conversation.user.receiverId === receiverIdToCheck) {
			// If a match is found, return the conversationId
			return conversation.conversationId;
		}
	}
	// If no match is found after checking all conversations, return null
	return null;
}
function containsUserId(activeUsers, id) {
    return activeUsers.some(user => user.userId === id);
}
const Dashboard = () => {
	...
	const fetchMessages = async (conversationId, receiver, isBusy) => {
		if (!isBusy && conversationId !== null) {
			alert("User is busy")
		}
		const id = findConversationIdByReceiverId(conversations,receiver.receiverId) 
					...
					<div>
						{
							conversations.length > 0 ?
								conversations.map(({ conversationId, user },index) => {
									return (
										<div className='flex items-center py-8 border-b border-b-gray-300' key={index}>
											<div className='cursor-pointer flex items-center' onClick={() => fetchMessages(conversationId, user, true)}>
							...
							users.map(({ userId, user }) => {
								...
								return (
									<div className='flex items-center py-8 border-b border-b-gray-300'>
										<div className='cursor-pointer flex items-center' onClick={() => fetchMessages('new', user, activeUsers.some(i => i.userId === user.receiverId))}>
											<div><img src={Img1} className="w-[60px] h-[60px] rounded-full p-[2px] border border-primary" /></div>
											<div className='ml-6'>
												<h3 className='text-lg font-semibold'>{user?.fullName}</h3>
												{
													activeUsers.some(i => i.userId === user.receiverId) ? "avaiable" : "busy" 
												}
												<p className='text-sm font-light text-gray-600'>{user?.email}</p>
											</div>
										</div>
